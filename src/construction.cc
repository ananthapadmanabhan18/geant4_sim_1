...
G4VPhysicalVolume *DetectorConstruction::Construct(){
    ...
    G4Box* solidTarget = new G4Box("sTarget", 0.5*m, 0.5*m, 5*cm);
    logicTarget = new G4LogicalVolume(solidTarget, water, "lTarget");
    // new G4PVPlacement(0, G4ThreeVector(0,0,30*m), logicTarget, "pTarget1", logicWorld, false, 0, checkOverlaps);

    TH2F* hist_2d = new TH2F("XY Distribution", "XY Distribution;X (cm);Y (cm)", 20, -5, 5, 20, -5, 5);

    // loop over all bins to place multiple targets
